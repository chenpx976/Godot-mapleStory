...
enum STATE { STAND, WALK, JUMP, JUMP_DOWN, CLIMP, CLIMPING }
enum STANCE { stand1, walk1, jump, rope }
@export var state = STATE.STAND
# 12 个绘制部分
var draw_part = ["Body", "Head", "Coat", "Pants", "Shoes", "Glove", "Weapon", "Shield", "Cape"]  # 手套
...
func getPosAtViewPort():
	var v = Vector2(
		characterBody.position.x + viewPortSize.x / 2, characterBody.position.y + viewPortSize.y / 2
	)
	return v
